...
def check_args(args):
    ...
    # info_gain checks
    if "info_gain" in args.name:
        if args.ig_fraction is None and args.ig_threshold is None:
            raise ValueError(
                "--ig_fraction or --ig_threshold must be specified for info_gain baselines"
            )
        if args.ig_fraction is not None and args.ig_threshold is not None:
            raise ValueError(
                "specify either --ig_fraction or --ig_threshold for info_gain baselines but not both"
            )
        if args.ig_arch is None:
            raise ValueError(f"specify architecture {ARCH}, for info_gain baselines")
        if args.ig_target is None:
            raise ValueError(f"specify target {TARGET} for info_gain")
    if args.ig_fraction is not None and not ("info_gain" in args.name):
        raise ValueError("--ig_fraction value only used for info_gain baselines")
    if args.ig_threshold is not None and not ("info_gain" in args.name):
        raise ValueError("--ig_threshold value only used for info_gain baselines")
    if args.ig_arch is not None and not ("info_gain" in args.name):
        raise ValueError("--ig_arch value only used for info_gain baselines")
    if args.ig_target is not None and not ("info_gain" in args.name):
        raise ValueError("--ig_target value only used for info_gain baselines")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="A threshold to apply on a infomation gain score (e.g., a value of 0.25 will only keep examples with information gain score over 0.25)",
    )
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="a fraction of metadata to keep according to information gain score (e.g., a value of 0.25 will keep the top 25 percent of examples in the pool by information gain score)",
    )
    ...
    parser.add_argument(
        ...
        required=False,
        choices=list(ARCH),
        help="kinds of features (b32 or l14) on which to run the information gain score filter",
    )
    ...
    parser.add_argument(
        ...
        required=False,
        choices=list(TARGET),
        help="target of the information gain score filter (txt or img)",
    )

    parser.add_argument(
        "--gcld3_en_filter",
        action="store_true",
        help="if set, the filter will be applied to the gcld3_en metadata files. This is only relevant for the clip_score and info_gain baselines.",
    )
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="A threshold to apply on a CLIP score with infogain (e.g., a value of 0.25 will only keep examples with CLIP score over 0.25)",
    )
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="a fraction of metadata to keep according to CLIP score with infogain (e.g., a value of 0.25 will keep the top 25 percent of examples in the pool by CLIP score)",
    )

    parser.add_argument(
        "--reverse",
        action="store_true",
        help="if set, the filter will be applied to the reverse metadata files. This is only relevant for the info_gain baselines.",
    )

    parser.add_argument(
        "--uniform_filter",
        action="store_true",
        help="if set, the filter will be applied to unifomly downsample the metadata files. This is only relevant for the info_gain baselines.",
    )
    ...
    parser.add_argument(
        ...
        required=False,
        default=1000,
        help="number of bins to use for the histogram in the info_gain baseline. This is only relevant for the info_gain baselines.",
    )
