...
def load_uids_with_info_gain_helper(
    fs_url: Tuple[Any, str], 
    key: str, 
    threshold: float, 
    gcld3_en_filter: bool,
    clip_score_key: str,
    ...
    if gcld3_en_filter:
        df = pd.read_parquet(url, columns=["uid", "text", key, clip_score_key], filesystem=fs)
        ...
        df = pd.read_parquet(url, columns=["uid", key, clip_score_key], filesystem=fs)

    if clip_score_threshold is not None:
        # apply clip score threshold
        df = df[df[clip_score_key] >= clip_score_threshold]
    ...
    return np.array(out_list, np.dtype("u8,u8"))
...
def get_threshold(
    metadata_dir_path: str, key: str, fraction: float, num_workers: int, reverse: bool = False
) -> float:
    ...
    fraction = fraction if not reverse else 1 - fraction
    df = load_metadata(metadata_dir_path, num_workers=num_workers, columns=[key])
    n = int(len(df) * fraction)
    threshold = -np.sort(-df[key].values)[n]
    print(f"threshold for {key} is {threshold}")
    print(f"max {key} is {df[key].max()}")
    print(f"min {key} is {df[key].min()}")
    print(f"mean {key} is {df[key].mean()}")
    print(f"std {key} is {df[key].std()}")
    print(f"total {key} is {len(df)}")
...
def downsample_df_by_ig(df: pd.DataFrame, f: float, B: int, ig_col: str = "ig") -> pd.DataFrame:
    """
    Args:
        df (pd.DataFrame):
        f (float):
            削減率（0 < f <= 1）。例: 0.05 なら元の約5%以下になる
        B (int):
            ヒストグラムのビン数。
        ig_col (str):
            分布に使う列名（デフォルト "ig"）
    ...
    Returns:
        pd.DataFrame: 削減後のデータフレーム（元の行 index を維持）。
    """
    N = len(df)
    M = int(np.ceil(f * N))

    counts, bin_edges = np.histogram(df[ig_col], bins=B)
    bin_idx = np.digitize(df[ig_col], bins=bin_edges[:-1], right=False)  # 1～B

    T = M / B

    print(f"downsampling {N} entries to {M} entries with {B} bins and f={f}")

    selected_indices = []
    for b in range(1, B+1):
        idx_b = df.index[bin_idx == b].to_numpy()
        n_b = len(idx_b)
        if n_b == 0:
            continue
        if n_b <= T:
            # 少ない領域は全保持
            selected_indices.extend(idx_b.tolist())
        else:
            # 多い領域は floor(T) 個だけランダムに選ぶ
            pick = np.random.choice(idx_b, size=int(np.floor(T)), replace=False)
            selected_indices.extend(pick.tolist())

    output_df = df.loc[selected_indices].copy()
    new_counts, new_bin_edges = np.histogram(output_df[ig_col], bins=B)

    # save histogram
    save_time_str = time.strftime("%Y%m%d-%H%M%S", time.localtime())
    save_name = f"histogram_{ig_col}_{str(int(f*100))}_{save_time_str}.npz"
    np.savez(
        save_name,
        counts=counts,
        bin_edges=bin_edges,
        new_counts=new_counts,
        new_bin_edges=new_bin_edges,
        bins=B,
        f=f,
        M=M,
    )
...
def load_uids_with_info_gain(
    ...
    num_workers: int,
    gcld3_en_filter: bool = False,
    clip_score_threshold: Union[float, None] = None,
    clip_score_fraction: Union[float, None] = None,
    reverse: bool = False,
    ...
    # clip score
    clip_score_key = "clip_l14_similarity_score"
    if arch == "b32":
        clip_score_key = "clip_b32_similarity_score"

    if clip_score_threshold is None and clip_score_fraction is not None:
        if arch == 'clip_score_threshold':
            raise NotImplementedError("clip_score_threshold is not implemented")
        # convert a fraction into a threshold
        clip_score_threshold = get_threshold(metadata_dir_path, clip_score_key, clip_score_fraction, num_workers)
    ...
    if arch == "b32":
        if target == "img":
            ...
            raise ValueError(f"Unknown target {target} for info gain")
    elif arch == "l14":
        if target == "img":
            key = "ig_l14_img"
        elif target == "txt":
        ...
        raise ValueError(f"Unknown arch {arch} for info gain")

    if uniform_filter:
        # uniform filter, downsample by information gain
        print(f"downsampling by {fraction} with {key} and {arch}")
        df = load_metadata(metadata_dir_path, num_workers=num_workers, columns=[key, "uid"])
        df = df.reset_index(drop=True)
        df = downsample_df_by_ig(df, fraction, num_bins, ig_col=key)
        print(f"downsampled to {len(df)} entries")
        inds_array = np.array(
            [(int(uid[:16], 16), int(uid[16:32], 16)) for uid in df["uid"].values],
            np.dtype("u8,u8"),
        )
    ...
    if threshold is None:
        # convert a fraction into a threshold
        threshold = get_threshold(metadata_dir_path, key, fraction, num_workers, reverse=reverse)

    fs, url = fsspec.core.url_to_fs(metadata_dir_path)
    parquet_paths = [(fs, str(x)) for x in fs.ls(url) if ".parquet" in x]

    worker = partial(
        load_uids_with_info_gain_helper,
        key=key,
        threshold=threshold,
        gcld3_en_filter=gcld3_en_filter,
        clip_score_key=clip_score_key,
...
def apply_filter(args: Any) -> None:
    ...
    elif args.name == "info_gain":
        print(f"threshold {args.threshold} and fraction {args.fraction}")
        uids = load_uids_with_info_gain(
            args.metadata_dir,
            args.ig_arch,
            args.ig_target,
            args.ig_threshold,
            args.ig_fraction,
            args.num_workers,
            gcld3_en_filter=args.gcld3_en_filter,
            clip_score_threshold=args.clip_score_threshold,
            clip_score_fraction=args.clip_score_fraction,
            reverse=args.reverse,
            uniform_filter=args.uniform_filter,
